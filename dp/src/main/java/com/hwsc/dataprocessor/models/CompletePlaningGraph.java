package com.hwsc.dataprocessor.models;

import com.google.common.collect.Lists;
import com.google.common.collect.Sets;
import com.hwsc.dataprocessor.CompleteGraph;
...
public class CompletePlaningGraph {
    ...
    public Graph<DWGNode, DWGEdge> trans() {
        initData();
        expandGraph();
        return dwGraph;
    }
    ...
    private void createPreNodesForNode(DWGNode node) {
        ...
        if (preNodeAboutSize > CompleteGraph.MAX_NEW_PRE_NODE_SIZE) {
            warn("may drop some node because of size is too big: (about){} (>{})",
                    preNodeAboutSize, CompleteGraph.MAX_NEW_PRE_NODE_SIZE);
        }
    ...
    private void combineServiceHelper(List<List<LeveledService>> services,
        ...
        if (services.size() == 0 || serviceCombinationResult.size() > CompleteGraph.MAX_NEW_PRE_NODE_SIZE)
            return;
